import type { TransportType, AuthConfig } from '../types';

/**
 * Configuration for generating "Try in Modelman" links
 */
export interface TryInModelmanConfig {
    name: string;
    transport: TransportType;
    command?: string;
    url?: string;
    auth?: AuthConfig;
}

/**
 * Generates a "Try in Modelman" URL from server configuration
 * 
 * @param config Server configuration
 * @param baseUrl Base URL of the Modelman app (default: current origin)
 * @returns A shareable URL that opens Modelman and prompts to add the server
 * 
 * @example
 * ```typescript
 * const link = generateTryInModelmanLink({
 *   name: "Weather MCP Server",
 *   transport: "http",
 *   url: "http://localhost:3000"
 * });
 * // Returns: "https://modelman.app/?try=eyJuYW1lIjoiV2VhdG..."
 * ```
 */
export function generateTryInModelmanLink(
    config: TryInModelmanConfig,
    baseUrl: string = window.location.origin
): string {
    // Validate configuration
    if (!config.name || !config.transport) {
        throw new Error('Config must include name and transport');
    }

    if (config.transport === 'stdio' && !config.command) {
        throw new Error('stdio transport requires a command');
    }

    if ((config.transport === 'sse' || config.transport === 'http') && !config.url) {
        throw new Error('SSE/HTTP transport requires a URL');
    }

    // Create minimal config object
    const minimalConfig: TryInModelmanConfig = {
        name: config.name,
        transport: config.transport,
        ...(config.command && { command: config.command }),
        ...(config.url && { url: config.url }),
        ...(config.auth && { auth: config.auth }),
    };

    // Encode configuration as base64
    const encoded = btoa(JSON.stringify(minimalConfig));

    // Generate URL with try parameter
    return `${baseUrl}/?try=${encoded}`;
}

/**
 * Generates HTML for a "Try in Modelman" button
 * 
 * @param config Server configuration
 * @param baseUrl Base URL of the Modelman app (default: current origin)
 * @returns HTML string for an anchor tag styled as a button
 * 
 * @example
 * ```typescript
 * const html = generateTryInmodelmanButton({
 *   name: "Weather MCP Server",
 *   transport: "http",
 *   url: "http://localhost:3000"
 * });
 * ```
 */
export function generateTryInModelmanButton(
    config: TryInModelmanConfig,
    baseUrl: string = 'https://modelman.app'
): string {
    const link = generateTryInModelmanLink(config, baseUrl);

    return `<a href="${link}" style="display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px; background: #6366f1; color: white; text-decoration: none; border-radius: 6px; font-weight: 600; font-family: system-ui, -apple-system, sans-serif; transition: background 0.2s;">
  <span>🚀</span>
  <span>Try in Modelman</span>
</a>`;
}

/**
 * Generates Markdown for a "Try in Modelman" button
 * 
 * @param config Server configuration
 * @param baseUrl Base URL of the Modelman app (default: current origin)
 * @returns Markdown string for a linked badge
 * 
 * @example
 * ```typescript
 * const markdown = generateTryInmodelmanMarkdown({
 *   name: "Weather MCP Server",
 *   transport: "http",
 *   url: "http://localhost:3000"
 * });
 * // Returns: "[![Try in Modelman](https://img.shields.io/badge/Try%20in-Modelman-6366f1)](https://modelman.app/?try=...)"
 * ```
 */
export function generateTryInModelmanMarkdown(
    config: TryInModelmanConfig,
    baseUrl: string = 'https://modelman.app'
): string {
    const link = generateTryInModelmanLink(config, baseUrl);

    return `[![Try in Modelman](https://img.shields.io/badge/Try%20in-Modelman-6366f1?logo=data:image/svg+xml;base64,)](${link})`;
}

/**
 * Decodes a "Try in Modelman" link and extracts the configuration
 * Useful for testing and debugging
 * 
 * @param url The full URL or just the encoded parameter
 * @returns The decoded server configuration
 */
export function decodeTryInModelmanLink(url: string): TryInModelmanConfig {
    try {
        // Extract the try parameter
        let encoded: string | null = null;

        if (url.includes('try=')) {
            const urlObj = new URL(url);
            encoded = urlObj.searchParams.get('try');
        } else {
            // Assume it's just the encoded string
            encoded = url;
        }

        if (!encoded) {
            throw new Error('No "try" parameter found in URL');
        }

        // Decode and parse
        const decoded = atob(encoded);
        const config = JSON.parse(decoded);

        return config;
    } catch (error) {
        throw new Error(`Failed to decode Try in Modelman link: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}

